  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center mb-4">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t('milestoneChecklist.title')}</h3>
      </div>

      {/* Completion Summary */}
      <div className={`p-4 rounded-lg border-2 ${getCompletionBackground(completionRate)} mb-4`}>
        <div className="text-sm text-gray-600">{t('milestoneChecklist.completionRate')}</div>
              ...
              <div className="flex-1">
                ...
                {milestone.evidence && (
                  <div className="text-sm text-gray-600 mt-1">
                    <span className="font-medium">{t('milestoneChecklist.evidence')}</span> {milestone.evidence}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="text-sm text-gray-700">
            {completionRate >= 80 ? (
              <span className="text-green-700 font-medium">
                {t('milestoneChecklist.excellentMessage')}
              </span>
            ) : completionRate >= 50 ? (
              <span className="text-yellow-700 font-medium">
                {t('milestoneChecklist.goodMessage')}
              </span>
            ) : (
              <span className="text-red-700 font-medium">
                {t('milestoneChecklist.needsImprovementMessage')}
              </span>
            )}
          </div>
        </div>
